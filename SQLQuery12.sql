...
SELECT ArtistName, COUNT(s.id) as numOfSongsPerArtist
FROM Song s
left JOIN Artist a on s.ArtistId = a.id
GROUP BY ArtistName;
...
SELECT COUNT(DISTINCT al.Label) as NumOfDiffLabels, Artist.ArtistName
FROM Album al
JOIN Artist ON al.ArtistId = Artist.id
